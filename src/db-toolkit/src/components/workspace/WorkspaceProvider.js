...
export function WorkspaceProvider({ children }) {
    ...
    try {
      ...
      if (result.success && result.workspaces) {
        ...
        if (result.workspaces.length > 0) {
          const sorted = [...result.workspaces].sort(
  ...
  const createWorkspace = useCallback(
    async (connectionId, connectionName, connectionType) => {
      try {
        ...
        const result = await ipc.createWorkspace(
          connectionId,
          connectionName,
          connectionType,
        );
  ...
  const closeMultipleWorkspaces = useCallback(
    async (workspaceIds) => {
      try {
        // Check for unsaved changes in any workspace
        const workspacesWithUnsavedChanges = workspaces.filter(
          (w) => workspaceIds.includes(w.id) && w.hasUnsavedChanges,
        );
        ...
        // If closing active workspace, switch to a remaining one first
        if (workspaceIds.includes(activeWorkspaceId)) {
          const remaining = workspaces.filter(
            (w) => !workspaceIds.includes(w.id),
          );
          if (remaining.length > 0) {
            const targetWorkspace = remaining[0];
            if (targetWorkspace.state?.activeRoute) {
        ...
        // Close all workspaces sequentially to avoid race conditions
        ...
        // Update state once after all deletions
        setWorkspaces((prev) =>
          prev.filter((w) => !workspaceIds.includes(w.id)),
        );

        return true;
      } catch (error) {
        console.error("Failed to close multiple workspaces:", error);
        return false;
      }
    },
    [activeWorkspaceId, workspaces, navigate, ipc],
  );
  ...
  const switchWorkspace = useCallback(
    (workspaceId) => {
      ...
      if (
        workspace?.state?.activeRoute &&
        workspace.state.activeRoute !== location.pathname
      ) {
  ...
  const updateWorkspaceState = useCallback(
    async (workspaceId, stateUpdates) => {
      try {
        const result = await ipc.updateWorkspaceState(
          workspaceId,
          stateUpdates,
        );
  ...
  const updateWorkspace = useCallback(async (workspaceId, updates, options = {}) => {
    try {
